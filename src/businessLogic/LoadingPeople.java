...
public class LoadingPeople {
	...
	public static Grad readGrad(Scanner sc) {
		...
		String school=sc.next().trim();
                ...
                float salaryF =Float.parseFloat(school);
                Date hiringDate;
		try {
			hiringDate = ft.parse(hd);
			newFulltime = new FullTime(id, name,hiringDate,school,dp,salaryF);
		} catch (ParseException e) {
			System.out.println("Unparseable using " + ft);
			e.printStackTrace();
		}
		...
		String school=sc.next().trim();
                ...
                int claseshoras=Integer.parseInt(ch);
                Date hiringDate;
		try {
			hiringDate = ft.parse(hd);
			newPartialtime = new PartialTime(cat,claseshoras,id,name,hiringDate,school,dp);
		} catch (ParseException e) {
			System.out.println("Unparseable using " + ft);
			e.printStackTrace();
		}
